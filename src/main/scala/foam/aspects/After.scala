...
object AfterState {
  def apply[A <: State](pointcut: Pointcut[A], base: NFA)(body: (StateJoinpoint[A], NFA) => (Option[(Token, State)], NFA)): NFA = {
...
object AfterToken {
  def apply[A <: Token](pointcut: Pointcut[A], base: NFA)(body: (TokenJoinpoint[A], NFA) => (Option[(State, Token)], NFA)): NFA = {
    Advice[A, NFA](pointcut, base)((prevBase, point) => {
