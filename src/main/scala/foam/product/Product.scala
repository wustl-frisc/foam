...
object Product {
  def apply(m1: NFA, m2: NFA, isIntersectionMachine: Boolean): NFA = {

    assert(m1.alphabet == m2.alphabet, "The alphabet of both machines must be the same for a Product Machine")

    val combinedStartState = CombinedState(m1.start, m2.start, isIntersectionMachine)
    ...
    do {
      ...
      for(((m1SrcState, m1Transition), m1DstStates) <- m1.transitions;
        ...
        if (m1Transition == m2Transition) {
          val combinedSrcState = CombinedState(m1SrcState, m2SrcState, isIntersectionMachine)
          val transitionToken = m1Transition

          for (m1DstState <- m1DstStates; m2DstState <- m2DstStates) {
            val combinedDstState = CombinedState(m1DstState, m2DstState, isIntersectionMachine)
