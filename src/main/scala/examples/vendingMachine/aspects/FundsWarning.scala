...
class FundsWarning extends Aspect[NFA] {
  def apply(nfa: NFA) = {
    ...
    Advice[(ValueState, Product), NFA](transitionKeyPointcut, nfa)((prevNFA, key) => prevNFA.addTransition(key, PrinterState("InsufficentFunds")))
  }
}
